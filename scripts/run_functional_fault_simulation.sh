...
show_help() {
    ...
    echo " -i               : Input stimulus for fault simulation"
...
if [ $OPTIND -eq 1 ]; then
   ...
   if [[ "$enable_vcd_dump" == "y" ||  "$enable_vcd_dump" == "Y" ]]; then 
    ...
    vcds=$(find -L ${MoW4EDA_FSIM_PATTERNS_DIR}/  -mindepth 1  -type f -name "*.evcd" ! \( -name "doc" -o -name "scripts" -o -name "tags" \) | xargs -I{} basename {} | sort) 
    if [ -z "${vcds}" ]; then
      echo "No vcds in your workspace." >&2
      exit 1
    fi
    echo "Select an input vcd file in ${MoW4EDA_FSIM_PATTERNS_DIR}:"
    select input_file in ${vcds}; do
    ...
    input_stimuli=${MoW4EDA_FSIM_PATTERNS_DIR}/${input_file}
...
cd ${MoW4EDA_FSIM_DIR}
echo "Running Fault Simulation with ${MoW4EDA_FSIM_TOOL}"
...
## clean up first 
if [[ -d "${MoW4EDA_FSIM_DIR}/image_functional" ]] ; then 
rm -rf ${MoW4EDA_FSIM_DIR}/image_functional
fi 
mkdir ${MoW4EDA_FSIM_DIR}/image_functional
echo "${MoW4EDA_FSIM_SCRIPTS_DIR}/elaborate_functional_fsim.sh"
${MoW4EDA_FSIM_SCRIPTS_DIR}/elaborate_functional_fsim.sh -l  ${log_dir}/elaboration_functional.log
...
echo "${MoW4EDA_FSIM_SCRIPTS_DIR}/run_functional_fsim_internal.sh  -evcd ${input_stimuli} -processes ${processes} ${dump_evcd_opt} -result-dir ${output_dir} ${clean_env_opt}"
${MoW4EDA_FSIM_SCRIPTS_DIR}/run_functional_fsim_internal.sh -evcd ${input_stimuli} -processes ${processes} ${dump_evcd_opt} -result-dir ${output_dir} -l ${log_dir} ${clean_env_opt} |  tee ${log_dir}/fsim_functional.log

if [[ $? -ne 0 ]] ; then 
echo "Functional Fault Simulation not completed"
exit 1
fi 
