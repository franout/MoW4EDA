...
#---------------------------------------------------------
   ...
   stil=${MoW4EDA_ATPG_STIL_DIR}/${stil}
...
result_dir=${MoW4EDA_FSIM_RESULTS_DIR}/${MoW4EDA_DESIGN}/${MoW4EDA_FAULT_MODEL}/structural/${stil_name}_${MoW4EDA_FSIM_STROBE_TYPE}
else
# the results dir
result_dir=${MoW4EDA_FSIM_RESULTS_DIR}/${MoW4EDA_DESIGN}/${MoW4EDA_FAULT_MODEL}/structural/${stil_name}
fi
...
if [[ ! -f "${MoW4EDA_FSIM_SCRIPTS_DIR}/${MoW4EDA_DESIGN}_structural.tcl" ]] ; then 
echo "Script for fault simulation of ${MoW4EDA_DESIGN}_structural is wrong"
exit 1
fi 
...
./${MoW4EDA_DESIGN}_simv -fsim  +vcd+verbose +timescale+override+1ns/1ps \
      -stim=type:stil \ 
      -stim=file:${stil_path} \ 
...
## cleanup 
if ${clean_opt} ; then 
   fmsh -blast ${MoW4EDA_DESIGN}
fi 
cmd="vc_fcm -tcl_script ${MoW4EDA_FSIM_SCRIPTS_DIR}/${MoW4EDA_DESIGN}_structural.tcl -connect -log ${log_dir}/fsim_internal_${stil_name}.log"
echo ${cmd}
${cmd}
