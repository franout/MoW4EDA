...
create_campaign -args "-full64 -daidir $::env(MoW4EDA_FSIM_DIR)/image_functional/$::env(MoW4EDA_DESIGN)_simv.daidir \
                    -campaign $::env(MoW4EDA_DESIGN)_fsim_$::env(MoW4EDA_FSIM_MODEL)_$::env(evcd_name) -sff $::env(MoW4EDA_FSIM_FAULT_LIST_FILE)  \
                    -dut $::env(MoW4EDA_FSIM_ELABORATION_TOP_LEVEL_NAME)  $::env(MoW4EDA_FSIM_FGEN_EXTRA_OPT) \
                    -overwrite -l $::env(log_dir)/fsim_internal_fgen_$::env(evcd_name).log"
 

create_testcases -args "-stim=verify_off -stim=inst:$::env(MoW4EDA_FSIM_EVCD_TOP_LEVEL_NAME) -stim=type:evcd -stim=file:$::env(evcd_path) -stim=clk:clk_i" -exec "$::env(MoW4EDA_DESIGN)_simv" -fault_injection_time "1ns"  -name test1 

coats -localhost  -verbose 
report -campaign $::env(MoW4EDA_DESIGN)_fsim_$::env(MoW4EDA_FSIM_MODEL)_$::env(evcd_name) -showmetadata -showmetadataid  -showsimdetails -showtimingid -report "$::env(result_dir)/before_fsim.sff" -overwrite
